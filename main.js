...
class Picture {
   constructor({
      ...
      } else texture = new THREE.TextureLoader().load(this.src)
   ...
   getViewType = (prop) => {
      ...
      if (window.innerWidth <= 600 && Object.keys(prop).includes('mobile'))
         viewType = 'mobile'

      for (let viewType of this.viewTypes) {
         if (window.innerWidth > Number(viewType)) continue

         if (Object.keys(prop).includes(viewType)) viewType = viewType
      }
   ...
   update = () => {
      ...
      for (let viewType of this.viewTypes) {
         ...
         if (
            ...
            !(
               Object.keys(this.scale).includes('mobile') &&
         ...
         if (
            ...
            !(
               Object.keys(this.linkedTo).includes('mobile') &&
               window.innerWidth <= 600 &&
               Number(viewType) >= 600
            )
         )
         ...
         if (
            ...
            !(
               Object.keys(this.alignment).includes('mobile') &&
               window.innerWidth <= 600 &&
               Number(viewType) >= 600
            )
         )
         ...
         if (
            ...
            !(
               Object.keys(this.offset).includes('mobile') &&
               window.innerWidth <= 600 &&
               Number(viewType) >= 600
            )
         )
         ...
         if (
            ...
            !(
               Object.keys(this.rotation).includes('mobile') &&
               window.innerWidth <= 600 &&
               Number(viewType) >= 600
            )
         )
      ...
      if (typeof this.offset[offsetViewType].x == 'number') {
         ...
         offsetXParts =
            this.offset[offsetViewType].x.match(/(-?\d*\.?\w+[%]?)/g)
         ...
         for (let part of offsetXParts) {
            if (part.endsWith('px')) {
               ...
               offsetX +=
                  (window.innerWidth * Number(part.replace('w%', ''))) / 100
      ...
      if (typeof this.offset[offsetViewType].y == 'number') {
         ...
         offsetYParts =
            this.offset[offsetViewType].y.match(/(-?\d*\.?\w+[%]?)/g)
         ...
         for (let part of offsetYParts) {
            if (part.endsWith('px')) {
               ...
               offsetY +=
                  (window.innerHeight * Number(part.replace('w%', ''))) / 100
            } else if (part.endsWith('e%')) {
...
function scroll() {
   ...
   const fazTerminalRect = document
      .getElementById('Faz-Terminal-Box')
...
function resizeWindow() {
   ...
   for (let i = 0; i < rows.length; i++) {
      ...
      if (
         ...
         (rowRect.width / 20) * boxData[box.id].column +
